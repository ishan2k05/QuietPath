...
## Phase 18: Real-Time Hardware GPS, World Map & External Sensory Telemetry (In Progress)
...
- [ ] **18.2 Google Maps Platform & World Cartography (Next Up)**: Integrate `google_maps_flutter` with Android API key, custom low-contrast sensory JSON styling (calm muted roads, highlighted green spaces, suppressed POI clutter), and multi-mode map view switching (Google Maps vector, Esri/OSM raster tiles, and pure low-stimulus vector canvas).
...
- [ ] **18.4 Environmental Ingestion into MCDA Engine**: Dynamically incorporate external sensory telemetry into `route_service.py`'s deterministic scoring formula ($C_e = \sum w_i S_i + H_e + A_e$).
- [ ] **18.5 Production Standalone APK Compilation**: Compile standalone release APK (`flutter build apk --release`).
- [ ] **18.6 Academic Viva Demonstration Deck**: Structured viva slides and demonstration script highlighting the 100% deterministic, explainable MCDA algorithm.
